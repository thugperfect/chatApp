...
const userController = {
    ...
    register:async (req,res)=>{
        try{
            ...
            if(user_name) throw res.status(400).json({msg: "Username already exists."})
        
            const emailSyntax = /[@gmail.com]$/
            const newEmail = email
            const email_check = await User.findOne({email:newEmail})
            if(email_check) throw res.status(400).json({msg: "Email already exists."})
        
            if(!emailSyntax.test(email) ) return res.status(400).json({msg: "Enter Correct Email"})
            const hash_password = await bcrypt.hash(password,12)
            if(password.length<7) throw res.status(400).json({msg: "Password must be more than 8 chars long"})
